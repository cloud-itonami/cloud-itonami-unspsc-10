(ns apiaryops.operation
  "ApiaryOperationActor -- one pollination-service coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ColonyAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Apiary Operations Governor
  ...
  each operation is auditable and checkpointed. An apiary's lifecycle
  is advanced by MANY ops (colony-health logging / pollination-route
  scheduling / harvest coordination), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human apiary coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [apiaryops.advisor :as advisor]
            [apiaryops.governor :as governor]
...
(defn build
  "Compiles an ApiaryOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `apiaryops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ColonyAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Apiary Operations Governor -- independent censor.
      (g/add-node :govern
